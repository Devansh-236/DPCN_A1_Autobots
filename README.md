...
| Average clustering | **0.101**, which is 6.6× the random-graph value |
...
| Mean degree of a neighbour | $\langle k^2 \rangle / \langle k \rangle$ = **3.46** vs $\langle k \rangle$ = 2.02, so 62% of respondents sit below their neighbours' average |
| Agreement rate across all answers | **80%**, so the class is overwhelmingly consensual |
...
1. **Encode.** Likert maps to a symmetric scale (`Strongly Disagree` = −2 … `Strongly Agree` = +2).
...
src/visualize.py               the ten report figures
